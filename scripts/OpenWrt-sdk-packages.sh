...
add_configured_package_prefix() {
  local package_prefix="$1"
  local package_name

  while IFS= read -r package_name; do
    add_artifact_package "$package_name"
  done < <(
    sed -n -E "s/^CONFIG_PACKAGE_(${package_prefix}[^=]*)=(y|m)$/\1/p" "$SDK_ROOT/.config"
  )
}
...
generate_artifact_filters() {
  ...
  add_configured_package_prefix nginx-mod-
